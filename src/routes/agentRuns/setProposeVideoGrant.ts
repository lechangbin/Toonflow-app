...
export function createSetProposeVideoGrantRouter(
  grants: Pick<GrantRuntime, "setProposeVideo">) {
  ...
  async (req, res, next) => {
    ...
    try {
      res.status(200).send(success(await grants.setProposeVideo({
...
export default createSetProposeVideoGrantRouter(createProjectSkillGrantRuntime({
