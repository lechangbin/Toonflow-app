...
export function createSetProposeDerivedAssetGrantRouter(
  grants: Pick<GrantRuntime, "setProposeDerivedAsset">) {
  ...
  async (req, res, next) => {
    ...
    try {
      const grant = await grants.setProposeDerivedAsset({ ...req.body, actorUserId });
...
export default createSetProposeDerivedAssetGrantRouter(createProjectSkillGrantRuntime({
