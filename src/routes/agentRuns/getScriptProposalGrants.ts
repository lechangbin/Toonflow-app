...
type Grants = Pick<ReturnType<typeof createProjectSkillGrantRuntime>,
  "inspectScriptProposals">;

export function createGetScriptProposalGrantsRouter(grants: Grants) {
  ...
  async (req, res, next) => {
    ...
    try {
      res.status(200).send(success(await grants.inspectScriptProposals(
        req.body.projectId, actorUserId)));
    } catch (error) {
...
export default createGetScriptProposalGrantsRouter(createProjectSkillGrantRuntime({
