...
import {
  createProjectSkillGrantRuntime, ProjectSkillGrantOwnershipError,
...
export function createSetProposeBillableImageGrantRouter(
  grants: Pick<GrantRuntime, "setProposeBillableImage">) {
  ...
  async (req, res, next) => {
    ...
    try {
      const grant = await grants.setProposeBillableImage({ ...req.body, actorUserId });
...
export default createSetProposeBillableImageGrantRouter(createProjectSkillGrantRuntime({
