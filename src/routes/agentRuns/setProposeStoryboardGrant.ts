...
import { createProjectSkillGrantRuntime, ProjectSkillGrantOwnershipError,
  ProjectSkillGrantVersionConflictError } from "@/skillRuntime/grants";

type GrantRuntime = ReturnType<typeof createProjectSkillGrantRuntime>;

export function createSetProposeStoryboardGrantRouter(
  grants: Pick<GrantRuntime, "setProposeStoryboard">) {
  ...
  async (req, res, next) => {
    ...
    try {
      res.status(200).send(success(await grants.setProposeStoryboard({
...
export default createSetProposeStoryboardGrantRouter(createProjectSkillGrantRuntime({
