...
export default router.post(
  ...
  async (req, res) => {
    ...
    const scripts = await getDatabaseRuntime().work(async (db) => {
      return await db("o_script").whereIn("id", scriptIds);
    });
    ...
    await getDatabaseRuntime().work(async (db) => {
      await db("o_script").whereIn("id", scriptIds).update({
        extractState: 2,
      });
    });
    ...
    async function persistGroupResult(result: GroupResult) {
      ...
      await getDatabaseRuntime().work(async (db) => {
        // 查询已有资产
        const existingAssets = await db("o_assets").where("projectId", projectId).select("id", "name");
        ...
        const allAssets = await db("o_assets").where("projectId", projectId).select("id", "name");
        ...
        await db("o_scriptAssets").whereIn("scriptId", batchScriptIds).delete();
        if (uniqueRows.length) {
          await db("o_scriptAssets").insert(uniqueRows);
        }

        // 本批成功的剧本状态更新为 1（成功）
        await db("o_script").whereIn("id", batchScriptIds).where("projectId", projectId).update({
          extractState: 1,
          errorReason: null,
        });
      });
    ...
    function processGroup(group: number[][][]) {
      group.map(async (itemIds) => {
        ...
        for (const scriptIds of itemIds as number[][]) {
          for (const scriptId of scriptIds) {
            ...
            if (!script) {
              ...
              await getDatabaseRuntime().work(async (db) => {
                await db("o_script").where("id", scriptId).where("projectId", projectId).update({ extractState: -1, errorReason: "未找到对应剧本" });
              });
            } else {
              // 查看状态是否为等待提取，仅对等待提取进行生成
              const item = await getDatabaseRuntime().work(async (db) =>
                db("o_script").where("projectId", projectId).where("id", scriptId).select("extractState").first(),
              );
        ...
        // 修改状态为正在提取中
        await getDatabaseRuntime().work(async (db) => {
          await db("o_script").where("projectId", projectId).whereIn("id", validScriptIds).update({
            extractState: 0, // 正在提取
          });
        });
        // 查询当前项目已有的资产列表，提供给 AI 参考
        const existingAssets = await getDatabaseRuntime().work(async (db) =>
          db("o_assets").where("projectId", projectId).select("name", "type"),
        );
        ...
        try {
          ...
          for (const { id, script } of validScripts) {
            ...
            await getDatabaseRuntime().work(async (db) => {
              await db("o_script").where("id", id).where("projectId", projectId).update({ extractState: -1, errorReason: u.error(e).message });
            });
          }
          return;
        }
        if (!collectedNew.length && !collectedExisting.length) {
          for (const { id } of validScripts) {
            ...
            await getDatabaseRuntime().work(async (db) => {
              await db("o_script").where("id", id).where("projectId", projectId).update({ extractState: -1, errorReason: "AI 未返回任何资产" });
            });
          }
          return;
        }
