...
export default router.post(
  ...
  async (req, res) => {
    ...
    const { data, assets2AudioData } = await getDatabaseRuntime().work(async (db) => {
      ...
      const assets2AudioData = await db("o_assetsRole2Audio")
    ...
    const result = await Promise.all(
      data.map(async (parent: any) => {
        const historyImages = await getDatabaseRuntime().work(async (db) => {
          return await db("o_image").where("assetsId", parent.id).andWhere("state", "已完成").select("id", "filePath");
        });
