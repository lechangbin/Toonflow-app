...
type ToolProjectionTransform = { kind: "tool-result-projection.v1"; sourceContentHash: string;
  strategy: "novel-text-prefix-128" | "novel-events-head-2" };
...
export interface ContextCandidateSource {
  ...
  compact?: { content: string; contentHash: string; transform: ToolProjectionTransform };
}
...
export interface ContextSourceEntry {
  ...
  authorityRank: number;
  transform?: ContextCandidateSource["transform"] | ToolProjectionTransform;
}
...
export interface ContextSourceSelection {
  ...
  compactionActions: Array<{ sourceId: string; action:
    "typed-projection" | "pagination" | "evidence-slice" | "tool-projection"
    | "deduplicated" | "omitted-over-budget" }>;
...
export function selectEligibleContextSources(
  ...
  const seen = new Map<string, { revision: string; contentHash: string; transform?: string; compact?: string }>();
  for (const source of candidates) {
    ...
    if (!IDENTIFIER.test(source.revision) || !HASH.test(source.contentHash)
      ...
      || (source.transform !== undefined && (source.transform.kind === "locatable-evidence-slice.v1"
        ...
        : source.transform.kind === "catalog-page.v1"
          ...
          : true))
      || (source.compact !== undefined && (source.category !== "toolResults"
        || source.transform !== undefined
        || !HASH.test(source.compact.contentHash)
        || createHash("sha256").update(source.compact.content).digest("hex") !== source.compact.contentHash
        || source.compact.transform.kind !== "tool-result-projection.v1"
        || source.compact.transform.sourceContentHash !== source.contentHash
        || !["novel-text-prefix-128", "novel-events-head-2"].includes(source.compact.transform.strategy)
        || estimateContextTokens(source.compact.content) >= estimateContextTokens(source.content)))) {
    ...
    if (previous !== undefined) {
      if (previous.contentHash !== source.contentHash || previous.revision !== source.revision
        ...
        || previous.compact !== JSON.stringify(source.compact)) {
    ...
    seen.set(source.id, { revision: source.revision, contentHash: source.contentHash,
      transform: JSON.stringify(source.transform), compact: JSON.stringify(source.compact) });
  ...
  for (const source of eligible) {
    const fullTokens = estimateContextTokens(source.content);
    const projection = fullTokens > remaining[source.category]
      && !request.requiredSourceIds.includes(source.id)
      && source.compact !== undefined
      && estimateContextTokens(source.compact.content) <= remaining[source.category]
      ? source.compact : undefined;
    const content = projection?.content ?? source.content;
    const contentHash = projection?.contentHash ?? source.contentHash;
    const transform = projection?.transform ?? source.transform;
    ...
    selected.push({ id: source.id, revision: source.revision, contentHash,
    ...
    if (projection) compactionActions.push({ sourceId: source.id, action: "tool-projection" });
