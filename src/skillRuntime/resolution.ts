...
export async function resolveSkillDependenciesInTransaction(
  tx: Knex.Transaction, input: { role: string; rootSkillIds: readonly string[] },
): Promise<SkillResolutionPlan> {
...
export function createSkillDependencyResolver(work: DatabaseWork) {
  return {
    async resolve(input: { role: string; rootSkillIds: readonly string[] }): Promise<SkillResolutionPlan> {
      return work((db) => db.transaction((tx) => resolveSkillDependenciesInTransaction(tx, input)));
    },
  };
}
