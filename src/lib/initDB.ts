...
export default async (knex: Knex, forceInit: boolean = false): Promise<void> => {
  const tables: TableSchema[] = [
    ...
    //Base Asset 结构化身份记录表（Issue #41：当前态、schema-versioned，独立于 o_assets.describe）
    {
      name: "o_assetIdentity",
      builder: (table) => {
        table.integer("id").notNullable();
        table.integer("assetsId").unsigned().notNullable().unique();
        table.integer("projectId");
        table.integer("schemaVersion").notNullable();
        table.text("identity"); //结构化身份 JSON（type/canonicalName/aliases/summary/scriptIds/evidence/baseline）
        table.integer("createTime");
        table.integer("updateTime");
        table.primary(["id"]);
        table.unique(["id"]);
