...
export default async (knex: Knex, dataRoot = getPath()): Promise<void> => {
  ...
  await addColumn("o_project", "videoVendorId", "string");
  ...
  await addColumn("o_agentTrace", "videoVendorRequestId", "text");
  await addColumn("o_agentTrace", "videoArtifactId", "text");
  await addColumn("o_agentVideoVendorRequest", "cancellationRequestedAt", "integer");
  ...
  //矫正提示词
  // Issue #41：剧本 Base Asset 提取改用 data/skills/asset-extraction 下的版本控制
  // Skill 模板；旧的 o_prompt 行与其 prompt/assetsList 等不一致字段一并清理。
  await knex("o_prompt")
    .whereIn("type", ["scriptAssetExtraction", "scriptAssetExtractionBatch"])
    .delete();
