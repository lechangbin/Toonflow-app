...
ADR-0013 adds time-bounded Agent Run execution ownership in `src/agentRuntime/lease.ts`: worker owner, process epoch, expiry, and monotonic fence are checked on every worker state write. A heartbeat can extend the same ownership without advancing the Run lifecycle version. Readiness recovery skips unexpired leases and classifies expired Runs through the T05 checkpoint policy; expiry alone never authorizes a Provider replay. Version-checked, idempotent cancellation persists user intent; only queued work can become terminal cancelled before the model-call boundary. Project-scoped list/inspect HTTP snapshots provide the backend reconnect source of truth independently of Socket lifetime. Web wiring, Provider abort/reconcile, and multi-process acceptance remain future work.

ADR-0014 introduces `src/controlledTools/` as the single execution module for the first read-only Agent Tools. A ToolDefinition revision freezes schemas, risk and execution policy; a ToolReceipt stores one Run-scoped operation's bounded result or safe failure. The Runtime validates input, checks the Run's Project, role, scope and live lease, authorizes the Novel before adapter execution, deduplicates by operation identity, and writes a receipt with a causally linked Trace. The configured Model receives only two typed read Tools; legacy Socket-bound Script Agent tools remain unchanged until their migration stage.

ADR-0015 adds a distinct Production Agent write scope for `upsert_derived_asset`. `src/controlledTools/derivedAssetWrite.ts` freezes one validated, Project-owner-scoped proposal and bounded effect preview in a durable ToolApproval. The HTTP adapter derives the approver from the authenticated token, while the existing Production Agent chat sidebar renders backend-provided approval actions. Approval rechecks target revision, state hash, ownership, equivalent visual state and expiry, then commits the Asset, change instruction, ToolReceipt, Run Output, Agent Checkpoint and Trace in one SQLite transaction. The legacy Socket Production Agent write Tool remains a separate compatibility path until its migration stage; this slice does not grant write authority to the read-only Script Agent scope.
