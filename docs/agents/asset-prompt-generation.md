...
The Production Agent is the exclusive owner of Derived Asset discovery and identifies only stable, reusable, whole-Asset variations. Extraction scope is expressed through composable Visual State Dimensions per Asset type: characters use `age_stage`, `wardrobe`, `grooming`, `morphology`, `surface_condition`, `effect`, and `status_presentation`; scenes use `time_of_day`, `weather`, `season`, `atmosphere`, `practical_lighting`, and `persistent_condition`; props use `condition`, `configuration`, `activation`, and `contents`. Camera, shot scale, framing, frame position, pose, action phase, expression, gaze, and eyeline never define a Derived dimension. A parent Asset allows zero to N Derived Assets with no fixed minimum or maximum; a state that co-occurs (such as a rainy night) is one composite variant whose `dimensions[]` lists every involved dimension, and dimension Cartesian products are never generated.

When the Agent creates or updates a Derived Asset it records a structured Derived Change Instruction with `dimensions[]`, Script `evidence`, parent traits to `preserve`, permitted `change` entries, and `exclude` entries. The ordinary name and description remain display content, not the executable contract. Existing single-`changeKind` records remain readable through a deterministic compatibility conversion into a one-element `dimensions[]` contract; the conversion performs no Text Model call, and the next normal update writes the new format back. Empty-description or parentless records require re-analysis.
