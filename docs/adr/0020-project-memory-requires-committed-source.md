...
Captured source identity and content are immutable. Revocation is an explicit, idempotent Project-scoped lifecycle transition bound to the expected Memory revision and command ID; revoked Memory is excluded from new ContextBundles, while the record remains until the authorized Project deletion transaction removes its evidence.
