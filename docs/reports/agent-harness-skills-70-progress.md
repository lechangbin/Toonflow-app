...
- 对旧版「青春成长导演叙事规划」纯参考文本增加显式适配器：固定来源 ID、规范化正文 SHA-256、严格 frontmatter、无 Tool/Capability 请求。管理员先预览，再通过正常 saveDraft 保存；适配器不自动发布或激活，来源修改后必须重新审查并更新锁定哈希。旧决策/执行 Skill 因依赖旧工具和回调不在允许列表。
...
`tests/legacySkillImport.test.ts` 另有 2 个定向用例，证明固定旧来源可通过新 Runtime 校验并保存为未激活草稿，变更来源或未知来源被拒绝。该测试不证明其他旧 Skill 可直接运行。

依赖闭包与环校验、资源按 ID 加载、有效权限交集、路由和高风险歧义暂停、管理 UI、其他旧 Skill 的逐个适配、Run 创建时自动绑定以及旧 Socket Agent 迁移尚未完成。当前管理 API 也未接前端编辑器；manifest 中的依赖/资源/权限是经结构校验的声明，不是已解析或已授权的执行能力；面试材料不得夸称完整 Skill Marketplace 或安全授权闭环。
...
5. 问：如何避免旧 Skill 导入时把指令文本误当成授权？答：适配器只允许一个经审查的纯叙事参考源，锁定规范化源码哈希并检查 frontmatter，把请求的 Tool/Capability 固定为空。结果先是管理员预览的草稿提案，保存、发布、激活仍是三个明确步骤。对含旧式工具/前端回调的决策、执行文件不做自动迁移；测试覆盖来源变化和未知来源拒绝。它证明的是兼容子集，不是旧 Skill 全量兼容。
