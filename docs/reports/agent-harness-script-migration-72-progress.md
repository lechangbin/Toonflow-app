...
`agentRunRuntime.test.ts`、`scriptHarnessPreparation.test.ts`、`contextBundle.test.ts` 的相关定向用例通过，覆盖准备失败回滚、唯一 Skill 冻结、并列拒绝、Bundle 修订来源、撤销拒绝、Tool grant 拒绝及未知 Model 容量前置失败；T15 路由回归此前通过。`yarn lint`（TypeScript noEmit）通过。未运行全量测试、构建、浏览器或真实 Provider。

下一切片需要使 Script Agent 的生产入口显式选择新 Run，并提供持久可信的 grant 来源；还需处理规划/Script 写工具、停止/重连与前后端契约。当前 `skillMode` 只在定向注入测试中启用，不得称为端到端迁移。
...
5. 问：为什么提供了 Skill 指令后还要额外配置 Tool grant？答：指令文本不是权限。`skillMode` 用 Run 冻结的 Skill ID 调用可配置的受控 Tool 闸门，判定仍要求 Skill 请求、Tool 能力与四层外部 grant 同时成立；测试中即使四层 grant 都允许，Skill 未声明 Tool 也会在适配器执行前拒绝。生产环境尚无可信 grant 配置和旧 Socket 迁移，故不能声称已有全面执行保护。
