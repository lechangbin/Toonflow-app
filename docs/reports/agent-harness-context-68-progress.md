...
4. 问：既然有成功的 ToolReceipt，为什么还要给 Trace 绑定 Step/Attempt？答：回执只能证明工具输出曾被提交，不能证明它属于哪次 Model Attempt、是否早于本次 Context 构造。来源加载器要求成功 Trace 可追溯到前置 Step；此前真实 Tool 路径漏传身份，手工 Trace 单测掩盖了接线缺口。现在真实调用写入身份并在 operation 重放时核对，定向测试证明后续 Step 可读取这份回执；仍未证明同一步多轮 Model 调用与恢复的完整语义。
