...
全新浏览器夹具复测：T21 当前组合的隔离假 Model/数据库环境中，首次运行 `checkScriptModelProposalBrowser.js` 被欢迎引导弹窗遮挡，30 秒后超时；手动点“跳过引导”后重跑通过，父 Run `a78576b5-7b20-4c14-8e46-6068ed990301`、审批 Run `be7859aa-bedc-4337-88d8-c15d9ce45e8a`。这是夹具未处理全新 profile 的前置 UI，不是 Agent 功能失败。`checkScriptHarnessBrowser.js` 已有跳过逻辑，现将同样的条件处理补入模型提案和 Owner 审批/拒绝夹具；未重跑全新 profile 的三条完整链，也未扩展 Production 覆盖。全部调用仍是本地假文本 Model，未使用付费 Provider。
