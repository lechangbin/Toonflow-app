...
模型 Tool 提案增量：隔离夹具的已发布 Skill 显式请求 `propose_script_workspace_write` 与 `propose:script-workspace`；浏览器先由 Owner 开启 Project grant，本地假 OpenAI 兼容模型仅在 `[propose-fixture]` 输入下发出函数调用。`checkScriptModelProposalBrowser.js` 观察父 Run `f3f8d970-0137-4cbe-bfc1-cfd3d3f0fb2b` 成功，Trace 包含 `tool.proposal.created`，待审卡明确显示来自该父 Run；批准前 `storySkeleton` 没有被提案写入。Owner 查看全文并批准审批 Run `155afc33-b552-4b92-bbed-89b61881699d` 后，服务端再读才出现精确候选正文。此用例补上 Script 模型 Tool → Skill/Project grant → 待审候选 → Owner 批准的本地假 Model 正路径，但未覆盖拒绝授权、恶意 Tool 参数、Production Vendor、进程故障及其他入口；不能宣称 T18 完成。

全新浏览器夹具复测：T21 当前组合的隔离假 Model/数据库环境中，首次运行 `checkScriptModelProposalBrowser.js` 被欢迎引导弹窗遮挡，30 秒后超时；手动点“跳过引导”后重跑通过，父 Run `a78576b5-7b20-4c14-8e46-6068ed990301`、审批 Run `be7859aa-bedc-4337-88d8-c15d9ce45e8a`。这是夹具未处理全新 profile 的前置 UI，不是 Agent 功能失败。`checkScriptHarnessBrowser.js` 已有跳过逻辑，现将同样的条件处理补入模型提案和 Owner 审批/拒绝夹具。随后以第二份全新临时数据库和全空浏览器 profile 一次跑通修正后的模型提案夹具，父 Run `edc87290-febd-48c5-86a6-17e0b99646a0`、审批 Run `863b82df-75f8-4a81-85c4-3d2a33309b6e`；仍未重跑全新 profile 的全部三条链，也未扩展 Production 覆盖。全部调用仍是本地假文本 Model，未使用付费 Provider。
