...
2026-09-26 Agnes 当前 Flash 模型适配切片（Issue #102）：用户确认免费全模型 key 与 Agnes 3.0 Flash、Image 2.5 Flash、Video 2.5 Flash。官方文档分别给出 Chat Completions、Image generation、OpenAI Videos-compatible 异步任务接口。本机 CLI 顺序发起 1 个短文本请求成功；第一次图片 CLI 输出了 0 字节文件，不能算成功，查因是脚本把空 `b64_json` 当作有效内容。修正脚本后重跑的 Image 2.5 Flash 1K 文生图返回托管 PNG URL，HEAD 核验 HTTP 200、`image/png`、789874 字节；Video 2.5 Flash 4 秒/720P 提交两次均明确返回 `video_queue_full` HTTP 503，未创建任务，不能说视频生成通过。Vendor 适配器现在声明三款新模型，Video 2.5 翻译现代 `mode/seconds/size/aspect_ratio` 并保留 V2.0 帧数路径。相关定向用例与 TypeScript 检查通过，Vendor manifest 已再生成。CLI canary 未经过 Toonflow 的 Project/Prompt Revision/受控 VendorRequest/Artifact 链路；视频实际完成和最终验收仍待测。

用户补充其已实测 Video 2.5 Flash 接受 Base64 图片。因此目录保留单图与首尾帧能力，适配器把生产命令的图片字节转为 Data URI，并以假网络单测固定 `first_frame`/`last_frame` 形态；不支持 V2.0 的中间关键帧。这里的 Base64 可用性是用户提供的实验事实，本轮独立 canary 因视频队列满仍未复验，也没有 Toonflow 业务链路的成功产物。

2026-09-27 Image 2.5 Flash 参考图合同实测（Issue #32）：技能 CLI 顺序执行单张、两张、六张 Base64 PNG 参考图请求，均得到非空 PNG；单张与两张落盘分别为 745121、716577 字节，六张为 886205 字节并核对 PNG 文件头与 SHA-256。七张参考图明确返回 HTTP 400，服务商错误说明 `7 provided, at most 6 allowed`。因此当前六张上限有真实请求证据；测试只证明这些输入数量和输出文件，不证明人物一致性、提示词质量或 Toonflow 资产编排。首次 0 字节 CLI 产物已排除，不纳入成功样本。模型目录与适配器回归继续使用假网络测试，真实参考图请求未经过项目持久化链路。

2026-09-27 参考图超限保护：适配器原来对超过模型目录上限的参考图静默截断，可能使用户以为全部素材参与了生成。现改为提交前明确拒绝；六张仍原样发送，七张不会触发供应商 POST。Agnes 适配器定向测试 25/25 与 App TypeScript 检查通过。这个保护仅阻止本地输入丢失，不替代上层配置校验、图像质量评估或真实生产链路验收。同日第三次最小 Video 2.5 Flash 文生视频提交仍被 `video_queue_full` 503 明确拒绝，没有任务 ID，停止重试；用户提供的 Base64 视频图片输入可用性仍单列为用户实测，不冒充本轮独立视频完成证据。

2026-09-27 使用用户授权的 Agnes key 做单并发配置化 Vendor 探针，来源为 T17 修订 `756a89b5`。密钥只从进程环境读取；配置注入内存 SQLite，退出即销毁，未写入仓库、日志、媒体文件或持久 Project。先通过 `loadVendorRuntime` 调用真实 Image 2.5 Flash 适配器：文生图得到 639105 字节 PNG，SHA-256 `557c28d735c63aae668e6a96ae3a843ea1f9794e237477c624da8569896aa99c`；一张本地 Base64 PNG 参考图得到 649285 字节 PNG，SHA-256 `7dec1a34e9a8c35dde68ac9d0ff85059a7d6df923868c033315bb28f10082464`。随后使用 `createConfiguredVendor` 的真实配置加载与 AI SDK 路径：Agnes 3.0 Flash 短文本精确返回预期标记；Image 2.5 Flash 单张参考图返回 851854 字节 PNG，SHA-256 `c42f5e6eae6e2cdead2f671d9013b12705b2119d53bf7f98b64c0231438b141f`，阶段回调为 `generating`。上述 PNG 均检查了文件头；只保存脱敏元数据，未把原图和输出作为可独立复算的仓库 Artifact，因此不能计入 T21 最终证据索引。

同轮以技能 CLI 对 Video 2.5 Flash 的一张 Base64 首帧做 dry-run：请求为 `mode=keyframe`、4 秒、`720P`，未输出原始图片字节；真实提交收到明确 `video_queue_full` 503，没有任务 ID。此次拒绝发生在排队阶段，不能证明 Base64 首帧被成功接受或被拒绝；用户先前的 Base64 可用反馈继续单独记录。真实视频完成、Provider CDN、Prompt Revision→受控请求意图→Generation Task→Artifact Revision→工作台读回，均待后续验收。

2026-09-27 隔离 Production AgentRun 真实文本探针：新增显式运行的 `scripts/agnesProductionReadCanary.ts`，只从 `AGNES_API_KEY` 环境变量取密钥，内存 SQLite 中构造 Project、Script、生产工作区、已发布只读 Skill、Owner 的 `read:production-workspace` grant、Agnes 配置及 `productionAgent:decisionAgent` 逻辑绑定。实际 `createAgentRuntime` 经 `createConfiguredVendor.openTextCall` 请求 Agnes 3.0 Flash；模型只收到 `get_production_workspace_text` 一个受控工具，探针把调用最多限制为两个模型步骤。脚本先运行默认目录预检，确认 `failed/contextMissing` 且模型调用数为零；随后配置化 Vendor 的 fixture `customModels` 为本次实验声明保守的 4096-token 预算。第二次 Run 结果为 `succeeded`、一次模型调用、一个成功读取回执、一个持久输出（本次 SHA-256 `28b77d3d9977cfb1198141d227a039851ae66c624fed71661453b69d6d7347ad`）、零其他工具回执和零 Video/Image VendorRequest；`yarn lint` 与 Production Run 定向用例通过。内存库退出销毁，不使用真实 Project 或持久媒体。

该探针可在同次运行中复现 Agnes 文本模型目录未声明 `contextWindowTokens` 导致的预模型阻断；内存自定义预算不修改内置供应商目录，也不声称 Agnes 官方上下文窗口就是 4096。[Agnes 官方开源 Preview 说明](https://github.com/AgnesAI-Labs/Agnes-3.0-Flash)明确称其 checkpoint 与生产/API checkpoint 不同，不能把 Preview 的容量直接填入本项目目录。因此真实默认配置的生产 Skill Run 仍有阻断项，需取得可靠 API 容量声明并增加目录/回归测试，或设计无容量声明时的显式安全降级。一次隔离成功不代表真实 Project→完整生产写入/生成、跨进程恢复、浏览器、视频或 T21 最终验收成功。
