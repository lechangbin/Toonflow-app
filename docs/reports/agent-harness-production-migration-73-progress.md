...
`tests/productionHarnessWorkspaceRead.test.ts` 覆盖独立 Tool 修订/角色/scope、跨 Project 剧本拒绝、重复行拒绝、缺失草稿、无效 JSON 与超长内容。`tests/productionHarnessGrants.test.ts` 覆盖默认拒绝、非 Owner 拒绝、版本冲突、撤销、Script 授权隔离和 HTTP actor 来源；本阶段上述 4 个单元用例通过，`scriptHarnessPreparation.test.ts` 的 1 个相关回归曾通过，`yarn lint`（TypeScript `--noEmit`）通过。未运行全量测试、构建、浏览器或真实 Provider。
