...
2026-09-27 隔离 Production AgentRun 真实文本探针：新增显式运行的 `scripts/agnesProductionReadCanary.ts`，只从 `AGNES_API_KEY` 环境变量取密钥，内存 SQLite 中构造 Project、Script、生产工作区、已发布只读 Skill、Owner 的 `read:production-workspace` grant、Agnes 配置及 `productionAgent:decisionAgent` 逻辑绑定。实际 `createAgentRuntime` 经 `createConfiguredVendor.openTextCall` 请求 Agnes 3.0 Flash；模型只收到 `get_production_workspace_text` 一个受控工具，探针把调用最多限制为两个模型步骤。结果为 Run `succeeded`、一次模型调用、一个成功读取回执、一个持久输出（SHA-256 `2caf3f36d86699aec713d0d2d1df603ebcd36ebc4e6263bc905681c0ab85b7bd`）、零其他工具回执和零 Video/Image VendorRequest；`yarn lint` 通过。内存库退出销毁，不使用真实 Project 或持久媒体。

该探针首先发现 Agnes 文本模型目录未声明 `contextWindowTokens`，Skill-mode Run 在模型调用前以 `contextMissing` 失败；为验证其余链路，脚本仅在测试返回的 Model target 上附加保守的 4096-token fixture 预算，不修改供应商目录，也不声称 Agnes 官方上下文窗口就是该值。因此真实默认配置的生产 Skill Run 仍有阻断项，需取得可靠容量声明并增加目录/回归测试，或设计无容量声明时的显式安全降级。一次隔离成功不代表真实 Project→完整生产写入/生成、跨进程恢复、浏览器、视频或 T21 最终验收成功。
