...
8n. 问：把视频命令准备从手动生成流程抽出来，是否就能让 Agent 安全生成视频？答：不能。原流程的前半段确实已有 Model、Capability、Prompt Profile 和图片输入校验，但后半段会创建生产记录并立即调用供应商；校验正确不等于费用获得授权，也不等于提交结果可恢复。我把准备逻辑收敛成同一接口供手动流程和未来受控流程复用，受控候选只保留目标状态与脱敏命令哈希。定向测试确认准备时没有生成任务或供应商请求，并模拟检查供应商配置期间 Prompt 被改动，候选会拒绝。仍需独立 Owner 审批、提交意图、未知结果对账和迟到视频处理，才可把它接入模型 Tool。证据：`src/video/production.ts`、`src/controlledTools/videoGenerationPreparation.ts`、`tests/videoOrchestrationIntegration.test.ts`、`tests/videoGenerationProposalContract.test.ts`。
