...
> 对应开放 Issue #74。仅记录已实现的停止回执、迟到消息栅栏、Production 上下文隔离和局部 Web 接缝。旧 Socket 生命周期、跨仓浏览器、真实进程恢复与 T21 验收未完成；不写简历或未测收益。
...
当前没有全量测试、构建、跨仓浏览器或真实 Provider 验收；T18 的 Issue #74 仍开放，不应以局部止损宣称兼容迁移完成。
