...
确定性执行壳逐角色给出 Skill 白名单，Tool 调用必须经过受限 port，产物和下一跳再过契约校验；T2 的 verifier 必须给出 verification 和 final。它只用于假角色试验，不是可持久恢复的生产 AgentRun，也不能防止 handler 绕过 port 直接访问系统。
