...
5. 问：这个阶段能说 Golden Eval 或消融结果了吗？答：不能。现在能冻结真实 18-case Golden manifest；测试只接了一个实际 AgentRuntime + Fake Model 的只读样例，不是 Golden 场景迁移。账本能留单 Run 耗时和结构覆盖，待评审清单仍将 72 个样本的业务硬门、rubric 与失败分类列为待判定；未采集真实费用，也没发布结果级成对报告。T19 真正消融仍被 T11 依赖阻塞，全量验收留到 T21。
...
14. 问：为什么 Run 成功后硬门仍是未评估？答：生产 Run 成功只说明执行生命周期完成，不证明 Golden case 要求的业务产物正确、敏感信息未泄露或权限没有越界。待评审清单把每例声明的硬门 ID、必需产物和 rubric 版本列出，但所有结果先保持未评估；一个只读 Fake Model Run 即便被记录为成功，也不能自动变成抽取、图像恢复或安全回归用例通过。后续必须由对应场景的真实产物和校验器给出证据，再提交可追溯的逐例判定。
...
17. 问：评审提交了产物 SHA-256，为什么还要再验？答：提交值只是声明。报告 v2 在调用方给出明确 `artifactRoot` 后逐条读取引用文件、限制真实路径在根目录内、复算已声明的产物哈希；一个文件不符就拒绝整份报告。未传根目录时 `evidenceFileCheckedRuns=0`，不显示为已验。证据：`evaluationArtifactVerifier.ts`、`evaluationPairedAssessmentReport.ts` 及篡改单测。
18. 追问：两侧文件哈希全对，是否就能宣布候选质量提升？答：不能。哈希只证明当前读取字节与声明一致，不证明文件来自指定生产 Run，也不证明硬门判定、人工评分或身份真实。报告仍把分差标为暂定，72-cell 真实执行、独立场景语义校验和同条件控制没有完成。
19. 问：服务商并发额度很低，评测如何限流和断点续跑？答：`executeVariant` 只接受一个冻结变体，先对全部 case 正文、集合和修订做预检；随后按清单 case、seed 顺序逐格等待生产 Run 终结并记账。可设 `maxNewCells` 控制单次新增量，重进时从 `evaluation.inspect` 的已复核记录跳过旧 cell；定向测试观察到局部 Fake Model 最大并发 1、续跑不重发。追问边界：这不是跨进程锁或全局配额器，多个调用方并发发起仍需外层单执行者约束；测试没调用真实 Provider，不能报实际成本、72-cell 覆盖或业务质量。证据：`evaluationAgentCase.ts` 与 `tests/evaluationAgentCase.test.ts`。

源码证据索引：`src/eval/evaluationRun.ts`（清单、写入、读取）、`src/eval/goldenEvaluationFreeze.ts`（真实 18-case 清单冻结）、`src/eval/evaluationCoverageReport.ts`（覆盖非质量报告）、`src/eval/evaluationAssessmentQueue.ts`（未判定硬门/rubric 队列）、`src/eval/evaluationAgentCase.ts`（真实 Runtime 接线）、`src/lib/initDB.ts`（两表与不可更新触发器）、`src/types/database.d.ts`（生成类型）、`tests/evaluationRun.test.ts`、`tests/evaluationAgentCase.test.ts` 与 `tests/goldenEvaluationFreeze.test.ts`（定向验证）、`docs/reports/agent-harness-evaluation-67-progress.md`（未完成边界）。
