...
> 当前仅覆盖已落地的生产指导 Run、只读工作区 Tool 和单资产图片提案。T17 生成黄金链路未完成；完整测试、浏览器与真实 Provider 验收留到 T21。只写可核验的实现与学习材料，不写简历或线上收益。
...
认证 Owner → 生产 HTTP start → 事务内选择并冻结 Skill → Run 获得租约 → 模型读取生产工作区（需 `read:production-workspace`）→ 模型可提出单资产图片候选（另需 `propose:billable-image`）→ 后端核验父 Run、租约和权限 → 创建待 Owner 审批的 T09 子 Run → Owner 之后独立批准/拒绝 → 批准路径再由 T09 ledger 控制 Vendor 请求。模型 Tool 本身没有生成或扣费权限。
...
| 提案复用 T09 子审批 | 模型直接调用 Vendor | 模型建议不等于 Owner 承担费用；多一层人工处理 | 假模型提案后 VendorRequest 为零，Owner 拒绝不改父 Run |
...
- [ ] 明确区分“Run 成功回答”和“子审批已处理/图片已生成”。
- [ ] 不把 11 个定向单测与 TypeScript 检查说成全量、浏览器或真实 Provider 验收。
