...
5b. 读 `src/controlledTools/storyboardWriteContract.ts`、`storyboardWriteApproval.ts` 和 `storyboardWriteEffect.ts`：空 Track 单分镜候选冻结目标，内部 Owner 决策在一个事务中提交分镜、关联及持久证据；尚未接入模型或 Web。
