...
> 当前仅覆盖已落地的生产指导 Run、只读工作区 Tool、单资产图片提案及复用 T09 的假 Provider 成功路径。T17 生成黄金链路未完成；完整测试、浏览器与真实 Provider 验收留到 T21。只写可核验的实现与学习材料，不写简历或线上收益。
...
| 提案复用 T09 子审批 | 模型直接调用 Vendor | 模型建议不等于 Owner 承担费用；多一层人工处理 | 假模型提案后 VendorRequest 为零；Owner 批准后假 Provider 才调用一次并提交 Asset |
